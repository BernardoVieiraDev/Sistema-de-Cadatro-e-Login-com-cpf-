...
import models.banco_usuarios as banco_usuarios
from models.cpf import Cpf  # type: ignore
from models.utils import carregar_imagem  # Importa a função de carregar as imagens
...
class RegistrationInterface:
    def __init__(self):
        ...
        def open_login_interface():
            self.root.destroy()
            import models.login_interface as login_interface
        ...
        self.label_title = tk.Label(
            self.frame, text="Cadastro", font=("Arial", 16, "bold"), bg="white"
        )
        self.label_title.pack(pady=10)

        self.label_name = tk.Label(
            self.frame, text="Nome:", font=("Arial", 12), bg="white"
        )
        self.label_name.pack(anchor="w")
        self.entry_name = tk.Entry(
            self.frame, font=("Arial", 12), width=30, relief="solid", bd=1
        )
        self.entry_name.pack(pady=5)

        self.label_password = tk.Label(
            self.frame, text="Senha:", font=("Arial", 12), bg="white"
        )
        ...
        self.btn_show_password = tk.Button(
            ...
            command=lambda: show_hide_password(
                self.entry_password, self.btn_show_password
            ),
        )
        self.btn_show_password.pack(side="left", padx=5)

        self.label_confirm_password = tk.Label(
            self.frame, text="Confirmar Senha:", font=("Arial", 12), bg="white"
        )
        ...
        self.btn_show_password_confirm = tk.Button(
            self.confirm_password_frame,
            image=self.hide_icon,
            bd=0,
            bg="white",
            command=lambda: show_hide_password(
                self.entry_confirm_password, self.btn_show_password_confirm
            ),
        )
        ...
        self.label_cpf = tk.Label(
            self.frame, text="CPF:", font=("Arial", 12), bg="white"
        )
        self.label_cpf.pack(anchor="w")
        self.entry_cpf = tk.Entry(
            self.frame, font=("Arial", 12), width=30, relief="solid", bd=1
        )
        self.entry_cpf.pack(pady=5)
        self.entry_cpf.bind(
            "<KeyRelease>", lambda event: Cpf.apply_cpf_mask(event, self.entry_cpf)
        )

        # Mensagem de CPF inválido
        self.label_invalid_cpf = tk.Label(
            self.frame, text="", font=("Arial", 10), fg="red", bg="white"
        )
    ...
    def get_input_entry(self):
        return (
            ...
            self.entry_confirm_password.get(),
        )
    ...
    def add_user(self):
        ...
        cpf_obj = Cpf(cpf)  # Cria o objeto Cpf para validar o CPF
        if cpf_obj.validar_cpf(cpf) == "CPF inválido!":
            self.label_invalid_cpf.config(text="CPF inválido!", fg="red")
            return
        ...
        if self.bd.check_existence_cpf(cpf):
            messagebox.showwarning(
                "Usuário já existe", "Já existe um usuário com esse CPF cadastrado."
            )
            return
